...
const cartReducer = (state = [], action) => {
    ...
    }else if(action.type == actionCart.delete){
        var current = state.find(x=>x.product_id == action.data.id);
        if(current != undefined){
            return state.map(cart => cart.product_id == action.data.id ? {...cart, amount: --cart.amount} : cart).filter(cart => cart.amount > 0);
        }
    } else if (action.type == actionCart.deleteAll) {
        var current = state.find(x => x.product_id == action.data.id);
        if (current != undefined) {
            return state.map(cart => cart.product_id == action.data.id ? { ...cart, amount: 0 } : cart).filter(cart => cart.amount > 0);
        }
    }
    else{
        return state;
